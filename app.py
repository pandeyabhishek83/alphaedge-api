...
BUNDLED_UNIVERSE_SOURCE = "BUNDLED_NSE_SNAPSHOT_2026-09-02"
BUNDLED_NIFTY_CONSTITUENTS = {
    "NIFTY 50": [
...
def _load_bundled_index_constituents(index_name):
    entries = BUNDLED_NIFTY_CONSTITUENTS.get(index_name, [])
    if not 45 <= len(entries) <= 55:
        raise LiveDataError(
            f"The bundled {index_name} snapshot is incomplete",
            status_code=503,
            code="INVALID_BUNDLED_UNIVERSE",
            details={"index": index_name, "count": len(entries)}
        )

    stocks = [
        (symbol, name, industry, index_name)
        for symbol, name, industry in entries
    ]
    source = (
        f"bundled://{BUNDLED_UNIVERSE_SOURCE}/"
        f"{index_name.lower().replace(' ', '-')}"
    )
...
def _download_index_constituents(index_name, source_urls):
    ...
    return _load_bundled_index_constituents(index_name)
...
def get_stock_universe(force=False):
    ...
    try:
        for index_name, urls in NIFTY_INDEX_URLS.items():
            if force:
                ...
                constituents, source_url = (
                    _load_bundled_index_constituents(index_name)
                )
    ...
    bundled_fallback = any(
        str(source).startswith("bundled://")
        for source in source_urls.values()
    )
    zerodha_validation = None
    data_source = "NSE_INDICES_OFFICIAL"
    cache_status = "FRESH"
    ...
    if bundled_fallback:
        data_source = BUNDLED_UNIVERSE_SOURCE
        cache_status = "BUNDLED_FALLBACK"
        zerodha_validation = {
            "status": "SKIPPED_NO_ACTIVE_SESSION",
            "missing_symbols": []
        }

        if kite and KITE_API_KEY and kite_access_token:
            try:
                master = _load_nse_instruments()
                missing_symbols = sorted(
                    stock[0]
                    for stock in combined
                    if stock[0] not in master["by_symbol"]
                )
                zerodha_validation = {
                    "status": "PASSED" if not missing_symbols else "PARTIAL",
                    "missing_symbols": missing_symbols
                }
                if not missing_symbols:
                    data_source = (
                        f"{BUNDLED_UNIVERSE_SOURCE}_ZERODHA_VALIDATED"
                    )
            except LiveDataError as exc:
                zerodha_validation = {
                    "status": "FAILED",
    ...
    universe_cache["cache_status"] = cache_status
    universe_cache["refresh_error"] = None
    universe_cache["data_source"] = data_source
    universe_cache["zerodha_validation"] = zerodha_validation
...
NSE_REGULAR_OPEN_MINUTE = 9 * 60 + 15
NSE_SCAN_START_MINUTE = 9 * 60 + 30
NSE_REGULAR_CLOSE_MINUTE = 15 * 60 + 30
MIN_LIVE_SIGNAL_SCORE = 85
MAX_SCAN_SIGNALS = 3
LONG_RSI_MIN = 55.0
LONG_RSI_MAX = 68.0
SHORT_RSI_MIN = 32.0
...
ALLOW_SHORT_SIGNALS = (
    os.environ.get("ALLOW_SHORT_SIGNALS", "false").strip().lower()
    == "true"
)
...
def _equity_market_session(now_ist=None):
    now_ist = now_ist or datetime.now(IST)
    minute_of_day = now_ist.hour * 60 + now_ist.minute
    regular_open = now_ist.replace(hour=9, minute=15, second=0, microsecond=0)
    scan_start = now_ist.replace(hour=9, minute=30, second=0, microsecond=0)
    regular_close = now_ist.replace(hour=15, minute=30, second=0, microsecond=0)

    if now_ist.weekday() >= 5:
        status = "MARKET_CLOSED"
        message = "NSE cash market is closed for the weekend"
    elif minute_of_day < 9 * 60:
        status = "PRE_MARKET"
        message = "NSE cash market has not opened yet"
    elif minute_of_day < NSE_REGULAR_OPEN_MINUTE:
        status = "PRE_OPEN"
        message = "NSE is in its pre-open session; live scanning starts at 09:30 IST"
    elif minute_of_day < NSE_SCAN_START_MINUTE:
        status = "OPENING_RANGE"
        message = (
            "NSE is open, but AlphaEdge waits until 09:30 IST for the "
            "opening range to settle"
        )
    elif minute_of_day >= NSE_REGULAR_CLOSE_MINUTE:
        status = "MARKET_CLOSED"
        message = "NSE regular cash-market session has closed"
    else:
        status = "OPEN"
        message = "NSE regular cash-market session is open"

    return {
        "market_open": status in ("OPENING_RANGE", "OPEN"),
        "scan_ready": status == "OPEN",
        "market_session": status,
        "session_message": message,
        "checked_at": now_ist.isoformat(),
        "regular_market_open": regular_open.isoformat(),
        "scan_start": scan_start.isoformat(),
        "regular_market_close": regular_close.isoformat()
    }
...
def _analyze_live_stock(symbol=None, instrument_token=None, days=5,
    ...
    rsi_bullish = rsi > 50
    rsi_strong = LONG_RSI_MIN <= rsi <= LONG_RSI_MAX
    volume_confirmed = relative_volume >= MIN_RELATIVE_VOLUME
    ...
    rsi_bearish = rsi < 50
    rsi_weak = SHORT_RSI_MIN <= rsi <= SHORT_RSI_MAX
    ...
    short_requirements = {
        ...
        "ema20_below_ema50": ema_bearish,
        "rsi_in_entry_zone": rsi_weak,
        "volume_confirmed": volume_confirmed,
        "momentum_negative": momentum_negative
    }
    long_qualified = all(long_requirements.values())
    short_qualified = all(short_requirements.values())

    if (
        long_qualified
        and long_score >= MIN_LIVE_SIGNAL_SCORE
        and long_score > short_score
    ):
        bias, setup, score = "BULLISH", "LONG_SETUP", long_score
    elif (
        short_qualified
        and short_score >= MIN_LIVE_SIGNAL_SCORE
    ...
    return {
        ...
        **freshness,
        ...
        "conditions": {
            ...
            "volume_confirmed": volume_confirmed,
            "momentum_positive": momentum_positive,
            "long_requirements_met": long_qualified,
            "short_requirements_met": short_qualified
        },
        "required_confirmations": {
            "long": long_requirements,
            "short": short_requirements
        },
        "score_breakdown": {
            "above_vwap": {"points": 20 if above_vwap else 0, "maximum": 20},
            "above_ema20": {"points": 15 if above_ema20 else 0, "maximum": 15},
            "above_ema50": {"points": 15 if above_ema50 else 0, "maximum": 15},
            "ema20_above_ema50": {"points": 15 if ema_bullish else 0, "maximum": 15},
            "rsi_above_50": {"points": 10 if rsi_bullish else 0, "maximum": 10},
            "rsi_in_entry_zone": {"points": 10 if rsi_strong else 0, "maximum": 10},
            "volume_confirmed": {"points": 10 if volume_confirmed else 0, "maximum": 10},
            "momentum_positive": {"points": 5 if momentum_positive else 0, "maximum": 5}
        },
        "scores": {
            "long": long_score,
            "short": short_score,
            "final": score,
            "minimum_required": MIN_LIVE_SIGNAL_SCORE
        },
        "signal": {
            "bias": bias,
            "setup": setup,
...
def get_market():
    ...
    """
    session = _equity_market_session()
    if not session["scan_ready"]:
        return {
            "available": False,
            "source": "none",
            "data_source": "ZERODHA_KITE",
            "data_status": session["market_session"],
            "price": None,
            "chg": None,
            "rsi": None,
            "ema20": None,
            ...
            "block_long": True,
            "block_short": True,
            "error": session["session_message"],
            "error_code": session["market_session"],
            "http_status": 200,
            **session,
            "note": session["session_message"]
        }
...
def score_stock(sym, name, sector, hist, mkt):
    ...
    try:
        ...
        return {
            ...
            # Kept as null for clients that still expect the legacy field.
            "atm": None
        }
...
def scan():
    ...
    try:
        ...
        requested_min_score = max(
            0,
            min(int(request.args.get("min_score", MIN_LIVE_SIGNAL_SCORE)), 100)
        )
        min_score = max(MIN_LIVE_SIGNAL_SCORE, requested_min_score)
    except (TypeError, ValueError):
        return jsonify({
            "success": False,
    ...
    requested_direction = request.args.get(
        "direction",
        "long"
    ).lower().strip()
    if requested_direction not in ("all", "long", "short"):
        return jsonify({
            "success": False,
    ...
    elif requested_direction == "short":
        direction = "disabled"
    else:
        direction = "long"
    ...
    if (
        not force
        and is_cache_valid()
        and _equity_market_session()["scan_ready"]
        ...
        if not mkt.get("available", False):
            if mkt.get("error_code") in (
                ...
                return jsonify({
                    "success": True,
                    "market_open": mkt.get("market_open", False),
                    "scan_ready": False,
                    "data_source": "ZERODHA_KITE",
                    ...
                    "short_signals_enabled": ALLOW_SHORT_SIGNALS,
                    "note": mkt.get("note")
                })
            return jsonify({
                "success": False,
        ...
        for symbol, name, sector, index_name, instrument, quote_key in resolved:
            ...
            try:
                ...
                per_share_risk = ATR_STOP_MULTIPLIER * atr
                stop = price - per_share_risk if is_long else price + per_share_risk
                target_1_distance = TARGET_1_R_MULTIPLE * per_share_risk
                target_2_distance = TARGET_2_R_MULTIPLE * per_share_risk
                target_1 = (
                    price + target_1_distance if is_long
                    else price - target_1_distance
                )
                target_2 = (
                    price + target_2_distance if is_long
                    else price - target_2_distance
                )
                score = int(analysis["scores"]["final"])
                conditions = analysis["conditions"]
                legacy_components = {
                    "market_trend": int(mkt.get("mkt_pts") or 0),
                    "sector_strength": 0,
                    "stock_momentum": 15 if conditions["momentum_positive"] else 0,
                    "vwap": 10 if conditions["above_vwap"] else 0,
                    "volume": 10 if conditions["volume_confirmed"] else 0,
                    "rsi_score": 10 if conditions["rsi_in_entry_zone"] else 0,
                    "option_chain": 0,
                    "risk_filters": 10 if conditions["long_requirements_met"] else 0
                }
                results.append({
                    ...
                    "instrument_type": "EQ",
                    "trade_instrument": (
                        f"{analysis['exchange']}:{analysis['instrument']}"
                    ),
                    "option_contract": "CASH",
                    ...
                    "risk_plan": {
                        "stop_atr_multiple": ATR_STOP_MULTIPLIER,
                        "target_1_r_multiple": TARGET_1_R_MULTIPLE,
                        "target_2_r_multiple": TARGET_2_R_MULTIPLE,
                        "position_sizing": (
                            "floor(max_rupee_risk / risk_per_share); account "
                            "capital and daily P&L are not available to this API"
                        ),
                        "order_execution": "NOT_AUTOMATED"
                    },
                    # Avoid rendering a literal null in older cash-signal clients.
                    "atm": "CASH",
                    "components": legacy_components,
                    "components_status": "LEGACY_DISPLAY_ONLY",
                    "score_breakdown": analysis["score_breakdown"],
                    "score_model": "CASH_MOMENTUM_V2",
    ...
    if not gated:
        if ALLOW_SHORT_SIGNALS:
            ...
            note = (
                "No fresh long cash-equity setup met the AlphaEdge "
                "score and market-gate criteria. Short signals are disabled."
            )
    elif len(gated) > MAX_SCAN_SIGNALS:
        note = (
            f"{len(gated)} setups qualified; showing only the top "
            f"{MAX_SCAN_SIGNALS} by score."
        )

    returned_signals = gated[:MAX_SCAN_SIGNALS]
    result_times = [
        item["last_data_time"]
        for item in returned_signals
        if item.get("last_data_time")
    ]
    ...
    return jsonify({
        ...
        "data_source": "ZERODHA_KITE",
        "universe_data_source": universe_cache["data_source"],
        "universe_cache_status": universe_cache["cache_status"],
        "universe_zerodha_validation": universe_cache["zerodha_validation"],
        ...
        "maximum_signals": MAX_SCAN_SIGNALS,
        "strategy": {
            "name": "CASH_MOMENTUM_V2",
            "scan_start_ist": "09:30",
            "long_rsi_range": [LONG_RSI_MIN, LONG_RSI_MAX],
            "minimum_relative_volume": MIN_RELATIVE_VOLUME,
            "minimum_reward_risk": TARGET_1_R_MULTIPLE,
            "cash_only": True,
            "research_only": True
        },
        "scanned": n,
...
def debug_market():
    ...
    try:
        ...
        results["universe_data_source"] = universe_cache["data_source"]
        results["universe_cache_status"] = universe_cache["cache_status"]
        results["universe_source_urls"] = universe_cache["source_urls"]
        results["universe_zerodha_validation"] = (
            universe_cache["zerodha_validation"]
        )
