...
def get_market():
    try:
        hist = None
        for ticker in ["^NSEI", "NIFTY50.NS", "^CNX500", "NIFTYBEES.NS"]:
            try:
                t = yf.Ticker(ticker)
        ...
        if hist is None or len(hist) < 2:
            # Return sensible neutral defaults if Nifty unavailable
            return dict(price=24000, chg=0, rsi=55, av20=True, av_vwap=True,
                       bread=True, mkt_pts=15, bias="NEUTRAL",
                       bear=0, bull=2, block_long=False, block_short=False,
                       note="Nifty data unavailable — using neutral defaults")
...
def scan():
    ...
    # Use cache if valid and has data
    if is_cache_valid() and cache["data"] and len(cache["data"]) > 0:
        results = cache["data"]
        mkt = cache["market"]
    else:
        # Clear bad cache
